...
short Actual_360::compute_daycount(const std::string &from,
                                   const std::string &to) {
...
short Thirty_360::compute_daycount(const std::string &from,
                                   const std::string &to) {
...
short Thirty_360::compute_daycount(const short years, const short months,
  ...
  return (360 * years) + 30 * (months - 1) +
         std::max<short>(0, 30 - days_from) + std::min<short>(30, days_to);
}
